...
`);
  ...
  function renderTab() {
    switch (S.activeTab) {
      ...
      case "queues":
        return renderPlaceholder("Queues", "Queues allow creating a list-based view of issues matching JQL conditions, for use in the <strong>Rich Filter Results</strong> gadget.");
      case "custom-values":
        return renderPlaceholder("Custom values", "Define custom numeric values for issues to use in calculations and charts.");
      case "custom-ratios":
        return renderPlaceholder("Custom ratios", "Create ratios based on custom values.");
      case "time-series":
        return renderPlaceholder("Time series", "Track changes in issue metrics over time.");
      case "misc":
          ...
          <tbody>${items.map((sf, i) => `
            <tr>
              ...
              <td style="text-align:right">
                ...
                <button class="rib del" onclick="PAGE.delSF(${i})" title="Delete">\xD7</button>
              </td>
            </tr>`).join("")}
          </tbody>
        </table>` : `<div class="es"><div class="es-i">\u{1F518}</div><p>No static filters defined.</p><button class="btn btn-p" onclick="PAGE.addSF()">Create static filter</button></div>`}`;
  }
    ...
    ${added.length ? `<table class="stbl" style="margin-bottom:18px">
          ...
          <tbody>${added.map((k) => `
            <tr>
              <td style="width:28px"><span class="dh2">\u283F</span></td>
              <td>${esc(k)}</td>
              <td><button class="rib del" onclick="PAGE.delDF('${esc(k)}')" title="Remove">\xD7</button></td>
            </tr>`).join("")}
          </tbody>
          ...
          <tbody>${items.map((sf, i) => `
            <tr>
              ...
              <td style="text-align:right">
                ...
                <button class="rib del" onclick="PAGE.delSmf(${i})" title="Delete">\xD7</button>
              </td>
            </tr>`).join("")}
          </tbody>
        </table>` : `<div class="es"><div class="es-i">\u2728</div><p>No smart filters defined.</p><button class="btn btn-p" onclick="PAGE.addSmf()">Create smart filter</button></div>`}`;
  }
    ...
    ${items.length ? `<table class="stbl" style="margin-bottom:16px">
          ...
          <tbody>${items.map((v, i) => `
            <tr>
              ...
              <td style="text-align:right"><button class="rib del" onclick="PAGE.delView(${i})">\xD7</button></td>
            </tr>`).join("")}
          </tbody>
        </table>` : '<div style="padding:16px 0"></div>'}
    <div class="vab">
  ...
  function renderPlaceholder(title, desc) {
    ...
    <div class="es"><div class="es-i">\u{1F6A7}</div><p>${title} \u2014 coming soon</p></div>`;
  }
  function bindTabBody() {
    ...
    (_i = q("#btnSaveMisc")) == null ? void 0 : _i.addEventListener("click", async () => {
      var _a2, _b2;
