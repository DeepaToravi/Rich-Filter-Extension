...
`;
  ...
  function pushFilter(statusVal, priorityVal) {
    ...
    (0, import_bridge.invoke)("updateFilterState", { richFilterId: FILTER_ID, filters }).catch(console.error);
  }
  ...
  async function load() {
    try {
      const stats = await (0, import_bridge.invoke)("getStats", { richFilterId: FILTER_ID });
      renderStats(stats);
    } catch (err) {
      document.getElementById("statsContent").innerHTML = `<div class="status-msg">\u26A0 Error: ${err.message || JSON.stringify(err)}</div>`;
