...
`);
  ...
  var state = {
    ...
    presets: [],
    richFilters: [],
    // all available rich filters
    rfConfig: null
    // currently selected rich filter config
  };
  var CSS = `
...
`;
  ...
  function refreshChips() {
    ...
    el.innerHTML = chips.map(
      (c) => `<span class="chip">${esc(c.label)}<span class="chip-x" onclick="APP.removeChip('${c.field}','${esc(c.v)}')">\xD7</span></span>`
    ).join("");
  }
  function makeDropdown(id, label, options, renderItem) {
    ...
    document.getElementById(`${id}-btn`).onclick = (e) => {
      ...
      if (!wasOpen) {
        ...
        (_a = document.getElementById(`${id}-search`)) == null ? void 0 : _a.focus();
      }
    };
    document.getElementById(`${id}-search`).oninput = (e) => {
      const q = e.target.value.toLowerCase();
      const filtered = options.filter(
        (o) => (typeof o === "string" ? o : o.name || o.key || o.displayName || "").toLowerCase().includes(q)
      );
  ...
  window.APP = {
    toggleArr(field, val, ddId, label) {
      ...
      state.filters[field] = cur.includes(val) ? cur.filter((v) => v !== val) : [...cur, val];
    ...
    setProject(key) {
      ...
      if (state.filters.project) {
        (0, import_bridge.invoke)("getComponents", { projectKey: state.filters.project }).then((cs) => {
    ...
    addSmartFilter() {
      showModal({
        ...
        onSave(vals) {
          ...
          (0, import_bridge.invoke)("saveSmartFilters", { richFilterId: FILTER_ID, filters: state.smartFilters }).catch(console.error);
          renderQF();
        }
      });
    },
    ...
    savePreset() {
      showModal({
        ...
        onSave(vals) {
          ...
          (0, import_bridge.invoke)("savePreset", { name: vals.psName, data: { ...state.filters } }).then(() => loadPresets()).catch(console.error);
        }
      });
    },
    loadPreset(name) {
      (0, import_bridge.invoke)("loadPreset", { name }).then((data) => {
        if (!data) return;
        state.filters = { ...data };
    ...
    deletePreset(name) {
      (0, import_bridge.invoke)("deletePreset", { name }).then(() => loadPresets()).catch(console.error);
    }
  };
  var DEFAULT_QF = [
    { name: "All Issues", jql: "" },
  ...
  function renderPresets() {
    ...
    if (!row) return;
    const btns = state.presets.map(
      (p) => `<span class="preset-btn" title="${esc(p.name)}" onclick="APP.loadPreset('${esc(p.name)}')">
      \u{1F4CB} ${esc(p.name)}
  ...
  async function loadPresets() {
    const list = await (0, import_bridge.invoke)("listPresets").catch(() => []);
    state.presets = list || [];
    renderPresets();
  }
  ...
  function showModal({ title, fields, onSave }) {
    ...
    overlay.querySelector("#modalSave").onclick = () => {
      ...
      fields.forEach((f) => {
        var _a;
        vals[f.id] = (((_a = document.getElementById(`modal-${f.id}`)) == null ? void 0 : _a.value) || "").trim();
      });
      onSave(vals);
      overlay.remove();
    };
  ...
  async function loadRichFilters(defaultRfId) {
    ...
    if (!sel) return;
    sel.innerHTML = '<option value="">Select a rich filter\u2026</option>' + (rfs || []).map((f) => `<option value="${f.id}">${f.name.replace(/"/g, "&quot;")}</option>`).join("");
  ...
  async function applyRichFilter(id) {
    ...
    const rf = await (0, import_bridge.invoke)("getRichFilter", { id }).catch(() => null);
    if (!rf) return;
    state.rfConfig = rf;
    FILTER_ID = id;
    if (badge) {
      badge.textContent = rf.name;
      badge.style.display = "";
    }
  ...
  async function init() {
    ...
    (0, import_bridge.invoke)("getProjects").then((ps) => {
      state.projects = ps || [];
    ...
    (0, import_bridge.invoke)("getSmartFilters", { richFilterId: FILTER_ID }).then((qf) => {
      state.smartFilters = qf || [];
      renderQF();
    }).catch(() => renderQF());
    renderQF();
    loadPresets();
    (0, import_bridge.invoke)("getFilterState", { richFilterId: FILTER_ID }).then((saved) => {
