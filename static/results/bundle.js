...
`);
  ...
  var sortDir = "desc";
  var visibleCols = ALL_COLUMNS.filter((c) => c.def).map((c) => c.id);
...
`;
  ...
  function cellFor(colId, issue) {
    ...
    switch (colId) {
      ...
      case "status": {
        const s = ((_a = f.status) == null ? void 0 : _a.name) || "\u2014";
        const c = STATUS_COLORS[s] || STATUS_COLORS.default;
        return `<span class="sbadge" style="background:${c.bg};color:${c.text}">${s}</span>`;
      }
      case "priority": {
        const p = ((_b = f.priority) == null ? void 0 : _b.name) || "\u2014";
        const col = PRIORITY_COLORS[p] || PRIORITY_COLORS.default;
        return `<span class="p-dot" style="background:${col}" title="${p}"></span>`;
      }
      case "assignee":
        return `<span class="td-name" title="${((_c = f.assignee) == null ? void 0 : _c.displayName) || ""}">${((_d = f.assignee) == null ? void 0 : _d.displayName) || "Unassigned"}</span>`;
      case "reporter":
        return `<span class="td-name">${((_e = f.reporter) == null ? void 0 : _e.displayName) || "\u2014"}</span>`;
      case "issuetype": {
        const t = ((_f = f.issuetype) == null ? void 0 : _f.name) || "\u2014";
        return `<span title="${t}">${TYPE_ICONS[t] || TYPE_ICONS.default}</span>`;
      }
      case "created":
        return `<span class="td-sm">${formatDate(f.created)}</span>`;
      case "updated":
        return `<span class="td-sm">${formatDate(f.updated)}</span>`;
      case "resolution":
        return `<span class="td-sm">${((_g = f.resolution) == null ? void 0 : _g.name) || "Unresolved"}</span>`;
      default:
        return "\u2014";
    }
  }
  ...
  function renderTable(data) {
    ...
    issues = [...issues].sort((a, b) => {
      ...
      if (sortField === "key") {
        ...
        av = ((_a = af.status) == null ? void 0 : _a.name) || "";
        bv = ((_b = bf.status) == null ? void 0 : _b.name) || "";
      } else if (sortField === "priority") {
        av = ((_c = af.priority) == null ? void 0 : _c.name) || "";
        bv = ((_d = bf.priority) == null ? void 0 : _d.name) || "";
      } else if (sortField === "assignee") {
        av = ((_e = af.assignee) == null ? void 0 : _e.displayName) || "";
        bv = ((_f = bf.assignee) == null ? void 0 : _f.displayName) || "";
      } else {
        av = af.created || "";
        bv = bf.created || "";
      }
    ...
    if (!issues.length) {
      wrap.innerHTML = `<div class="empty"><div class="empty-icon">\u{1F50D}</div><div class="empty-text">No issues found</div><div class="empty-sub">Adjust filters in the Rich Filter Controller</div></div>`;
      return;
    }
    ...
    const headers = visibleCols.map((c) => {
      ...
      return `<th style="width:${w}" class="${sortClass}" ${sortable ? `onclick="window._sortBy('${c}')"` : ""}>${(col == null ? void 0 : col.label) || c}</th>`;
    }).join("");
    const rows = issues.map(
      (issue) => `<tr>${visibleCols.map((c) => `<td>${cellFor(c, issue)}</td>`).join("")}</tr>`
    ).join("");
    wrap.innerHTML = `<table><thead><tr>${headers}</tr></thead><tbody>${rows}</tbody></table>`;
  }
  function renderColPicker() {
    const picker = document.getElementById("colPicker");
    picker.innerHTML = `<div class="col-picker-title">Choose columns</div>` + ALL_COLUMNS.map(
      (c) => `<label class="col-item">
  ...
  function saveColumns() {
    (0, import_bridge.invoke)("saveColumnConfig", { richFilterId: FILTER_ID, columns: visibleCols }).catch(console.error);
  }
  window._openIssue = (key) => {
    const url = siteUrl ? `${siteUrl}/browse/${key}` : null;
    if (url) import_bridge.router.open(url).catch(() => window.open(url, "_blank"));
  };
  function mount() {
  ...
  async function load() {
    try {
      const savedCols = await (0, import_bridge.invoke)("getColumnConfig", { richFilterId: FILTER_ID });
      if (savedCols && Array.isArray(savedCols) && savedCols.length > 0) {
        visibleCols = savedCols;
      }
      const [data, info] = await Promise.all([
        (0, import_bridge.invoke)("getIssues", { richFilterId: FILTER_ID }),
        (0, import_bridge.invoke)("getSiteInfo").catch(() => null)
      ]);
      siteUrl = (info == null ? void 0 : info.baseUrl) || "";
      renderTable(data);
    } catch (err) {
      document.getElementById("resultWrap").innerHTML = `<div class="status-msg">\u26A0 Error loading issues: ${err.message || JSON.stringify(err)}</div>`;
